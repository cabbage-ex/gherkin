defmodule Gherkin.Parsers.RuleParser do
  ...
  def build_rule(rule, all_lines) do
    ...
    case line do
      %{text: "Background:" <> _} = _ ->
        {updated_rule, remaining_lines} = BackgroundParser.build_background(updated_rule, remaining_lines)
  ...
  defp build_scenario(scenario_name, line_number, tags, rule, remaining_lines) do
    ...
    {updated_scenario, remaining_lines} = DescriptionParser.build_description(new_scenario, remaining_lines)
    {updated_scenario, remaining_lines} = ScenarioParser.build_scenario(updated_scenario, remaining_lines)
  ...
  defp build_scenario_outline(scenario_name, line_number, tags, rule, remaining_lines) do
    ...
    {updated_scenario_outline, remaining_lines} = DescriptionParser.build_description(new_scenario_outline, remaining_lines)
    {updated_scenario_outline, remaining_lines} = ScenarioParser.build_scenario_outline(updated_scenario_outline, remaining_lines)
